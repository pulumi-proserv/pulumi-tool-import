...
5. `outputs` and `__pulumi_raw_state_delta` are built by the bridge (Task 4b below), falling back to direct attribute renaming with no delta when no provider schema is available.
...
### Task 4b: Build outputs and the raw state delta through the bridge

**Files:**
- Create: `pkg/state_injector_outputs.go`
- Create: `pkg/state_injector_outputs_test.go`
- Modify: `pkg/state_injector.go` (call it from `buildInjectedResource`)

**Interfaces:**
- Consumes: `ProviderWithMetadata`, `LookupProviderForPulumiType`, `MapTFAttributesToPulumi` (fallback), `NonImportableResource`.
- Produces: `func BuildOutputsWithDelta(ctx context.Context, prov *ProviderWithMetadata, tfType string, id string, attrs map[string]interface{}) (map[string]interface{}, bool, error)` — the returned bool reports whether a delta was produced.

**Why this exists.** The provider's own create path builds Pulumi outputs from a Terraform instance state and then attaches a delta (`pkg/tfbridge/provider.go:1360-1375`). Doing the same, with the sidecar's attributes standing in for the provider's response, gets the bridge's real TF→Pulumi conversion — `MaxItems=1` flattening, sets, assets, secret marking — and the delta from the same operation, with the bridge's `turnaroundCheck` refusing to write one that does not recover byte-for-byte.

**Sensitive values must already be substituted into `attrs` before this is called**, or the delta will enshrine `(sensitive)`. `redactedAttributes` is keyed by Terraform attribute name, so that substitution happens on the raw attribute map.

- [ ] **Step 1: Write the failing test**

Create `pkg/state_injector_outputs_test.go`. It needs a real loaded provider. `pkg/schema_fields_test.go:29` already has `loadAWSProvider(t)`, which returns a `map[providermap.TerraformProviderName]*ProviderWithMetadata` from the `testdata/bucket_state.json` fixture using the cache TestMain pre-warms. Add this single-provider wrapper to `pkg/schema_fields_test.go` next to it — the "find the AWS provider in the map" loop is already repeated three times in that file, so factoring it out serves the existing tests too:

```go
// awsProviderForTest returns the AWS provider itself, rather than the map of
// every provider in the fixture.
func awsProviderForTest(t *testing.T) *ProviderWithMetadata {
	t.Helper()
	for _, prov := range loadAWSProvider(t) {
		if prov.Name == "aws" {
			return prov
		}
	}
	require.FailNow(t, "AWS provider not found")
	return nil
}
```
...
func TestBuildOutputsWithDelta_SimpleType(t *testing.T) {
	t.Parallel()
	...
	outputs, hasDelta, err := BuildOutputsWithDelta(context.Background(), prov,
		"aws_vpn_gateway_route_propagation", "vgw-1_rtb-1", attrs)
	require.NoError(t, err)

	// The bridge does the renaming, so Pulumi property names appear.
	assert.Equal(t, "rtb-1", outputs["routeTableId"])
	assert.Equal(t, "vgw-1", outputs["vpnGatewayId"])

	// A delta should be produced for a provider on the v2Resource2 path.
	assert.True(t, hasDelta, "expected a raw state delta")
...
func TestBuildOutputsWithDelta_NoProviderFallsBack(t *testing.T) {
	...
	outputs, hasDelta, err := BuildOutputsWithDelta(context.Background(), nil,
		"aws_vpn_gateway_route_propagation", "vgw-1_rtb-1",
		map[string]interface{}{"route_table_id": "rtb-1"})
	require.NoError(t, err)

	// Without a schema, attributes are renamed directly and no delta is written.
	assert.Equal(t, "rtb-1", outputs["routeTableId"])
...
Note the fixture is `testdata/bucket_state.json`, an S3 state, so the AWS provider it loads carries the whole AWS schema — `aws_vpn_gateway_route_propagation` resolves from it fine.

- [ ] **Step 2: Run the tests to verify they fail**

Run: `go test ./pkg/ -run TestBuildOutputsWithDelta -v`
Expected: FAIL — `undefined: BuildOutputsWithDelta`.

- [ ] **Step 3: Write the implementation**

Create `pkg/state_injector_outputs.go` (with the licence header):
...
// BuildOutputsWithDelta converts recorded Terraform attributes into Pulumi
// outputs the way the bridge itself does on create, and attaches the raw state
// delta that goes with them.
//
// Mirrors pkg/tfbridge/provider.go:1360-1375, with the sidecar's attributes
// standing in for the provider's response. Going through the bridge rather than
// renaming properties by hand gets MaxItems=1 flattening, set handling, asset
// translation and secret marking for free, and the delta is computed by the same
// code that validates it: RawStateInjectDelta refuses to write a delta that does
// not recover the original raw state byte for byte.
//
// The second return value reports whether a delta was written. Providers not on
// the PlanResourceChange path return an instance state without a typed value,
// for which the bridge writes no delta; injection is still correct in that case,
// it just leaves the resource on the pre-delta reconstruction path.
//
// Sensitive attributes must already be resolved to their real values in attrs.
// A "(sensitive)" placeholder here would be baked into the delta.
func BuildOutputsWithDelta(
	ctx context.Context,
	...
	if prov == nil {
		return MapTFAttributesToPulumi(attrs, nil), false, nil
	}

	res := prov.P.ResourcesMap().Get(tfType)
	if res == nil {
		return MapTFAttributesToPulumi(attrs, nil), false, nil
	}

	var fields map[string]*tfbridge.SchemaInfo
	if ri := prov.Resources[tfType]; ri != nil {
		fields = ri.Fields
	}

	istate, err := res.InstanceState(id, attrs, nil)
	if err != nil {
		return nil, false, fmt.Errorf("building Terraform state for %s: %w", tfType, err)
	}

	props, err := tfbridge.MakeTerraformResult(
		ctx, prov.P, istate, res.Schema(), fields, nil, true)
	if err != nil {
		return nil, false, fmt.Errorf("converting %s state to Pulumi outputs: %w", tfType, err)
	}
	...
	return props.Mappable(), hasDelta, nil
}

var _ = resource.PropertyMap{}
```

Remove the trailing `var _` once the real code compiles; it is there only as a reminder that `props` is a `resource.PropertyMap` and needs converting to a plain map for the state document. If `Mappable()` does not round-trip secrets and assets the way the state file needs, use the same serialization `PatchState` relies on rather than inventing one — check how `pkg/state_patcher.go` writes secret envelopes and match it.
...
If `hasDelta` is false for a bridged AWS resource, that is a real finding, not a test to weaken: it means this provider is not on the `v2Resource2` path. Record it, and confirm by checking whether `res.InstanceState(...)` returns a value implementing `shim.InstanceStateWithTypedValue`.

- [ ] **Step 5: Add a delta round-trip test for a nested-block type**

Pick an AWS type with a nested block (`aws_vpn_connection` has `tunnel1_log_options`; confirm against the loaded schema) and assert that `tfbridge.UnmarshalRawStateDelta` followed by `Recover` reproduces the original attributes. This is the test that answers whether complex types survive injection:

```go
func TestBuildOutputsWithDelta_NestedBlockRoundTrips(t *testing.T) {
	t.Parallel()
	prov := testAWSProvider(t)

	// Attributes for a type whose schema has a nested block.
	attrs := map[string]interface{}{ /* fill from the real schema */ }

	outputs, hasDelta, err := BuildOutputsWithDelta(context.Background(), prov,
		"aws_vpn_connection", "vpn-1", attrs)
	require.NoError(t, err)
	require.True(t, hasDelta)

	props := resource.NewPropertyMapFromMap(outputs)
	delta, err := tfbridge.UnmarshalRawStateDelta(props["__pulumi_raw_state_delta"])
	require.NoError(t, err)

	recovered, err := delta.Recover(resource.NewObjectProperty(props))
	require.NoError(t, err, "delta must recover the raw state it was computed from")
	require.NotNil(t, recovered)
}
```

- [ ] **Step 6: Call it from the injector**

In `pkg/state_injector.go`, replace the `obj["outputs"] = MapTFAttributesToPulumi(...)` line in `buildInjectedResource` with a call to `BuildOutputsWithDelta`, passing attributes **after** sensitive values have been substituted. This means moving secret resolution ahead of output construction: resolve `redactedAttributes` into a copy of `r.Attributes` keyed by Terraform name, then build. Report the count of resources for which no delta was produced in `InjectResult` so the command can print it.

Add to `InjectResult`:

```go
	// NoDelta counts resources injected without a raw state delta, because
	// their provider is not on the PlanResourceChange path. They fall back to
	// the bridge's pre-delta state reconstruction.
	NoDelta int
```

- [ ] **Step 7: Run the full package tests**

Run: `go test ./pkg/ -v 2>&1 | tail -30`
Expected: PASS. The earlier injector tests that assert `NotContains(r, "__pulumi_raw_state_delta")` now need updating — those assertions were written when omitting the delta was the design. Change them to assert the delta is **present** when a provider is supplied, and absent only in the no-provider fallback.

- [ ] **Step 8: Commit**

```bash
git add pkg/state_injector_outputs.go pkg/state_injector_outputs_test.go pkg/state_injector.go pkg/state_injector_test.go
git commit -m "feat(inject): build outputs and raw state delta through the bridge"
```
