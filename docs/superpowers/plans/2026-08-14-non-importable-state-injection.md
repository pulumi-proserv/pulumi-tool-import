...
7. **Secret substitution: outputs are keyed by Pulumi name, `redactedAttributes` by Terraform name.** `redactedAttributes` maps a *Terraform* attribute name to the stack config key holding its real value, while `pulumiOutputs` uses *Pulumi* property names. Connect them with `PulumiToTFNames` (Task 2) over the type's `GetSchemaFieldInfo`. Substitute each resolved value into the outputs. A redacted attribute that cannot be resolved is a hard error, as for inputs.

8. **A delta that embeds `(sensitive)` is dropped, not repaired.** The digest computes the delta from redacted attributes, so where the bridge fell back to a `Replace` node the literal `(sensitive)` is baked into raw JSON. Substituting the real value into the outputs does **not** change what such a node recovers — it would reconstruct Terraform state containing the placeholder — and `validateRecover` would not catch it, because `Recover` succeeds and only the *value* is wrong. So: if a resource has any `redactedAttributes` **and** the marshalled delta contains the literal string `(sensitive)`, drop the delta for that resource. Losing the delta costs the pre-delta reconstruction path, which is previously-correct behaviour; keeping it would write a placeholder into raw state.

9. **After substitution, the delta is validated and dropped if it fails.** `validateRecover` (`pkg/state_patcher.go:1448`) already does exactly this check and needs no provider. A delta that no longer applies is removed for that resource. Count both drop paths in `InjectResult.NoDelta`.
